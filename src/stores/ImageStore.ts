...
function getInitialState(isImport?: boolean): ImageStoreState {
...
function reduce(state: ImageStoreState, action: Action): ImageStoreState {
  if (action.type == 'image-state-loaded') {
    ...
    _.forOwn(newState.images, (image, imageName) => {
      ...
      newState = deleteImageIfUnused(newState, imageName);
  ...
  switch (action.type) {
    case 'image-delete-from-pot': {
      ...
      newState = deleteImageIfUnused(newState, action.imageName, action.potId);
      persist(newState);
      return newState;
    }
    case 'pot-delete': {
      ...
      for (const name of action.imageNames) {
        ...
        newState = deleteImageIfUnused(newState, name, action.potId);
      }
      persist(newState);
      return newState;
    }
    ...
    case 'loaded': { // Pots loaded
      ...
      _.forOwn(state.images, (image, imageName) => {
        ...
        newImage.pots = potsUsingImage(imageName);
...
export function getImageState(state: ImageStoreState, name: string): ImageState | null {
...
function deleteImageIfUnused(state: ImageStoreState, imageName: string, exceptForPot?: string): ImageStoreState {
  ...
  if (image.pots.length === 0) {
    ...
    // being used.
    const pots = potsUsingImage(imageName).filter((p) => p !== exceptForPot); // double check
    if (pots.length !== 0) {
...
function potsUsingImage(image: string): string[] {
...
async function loadInitial(isImport?: boolean) {
...
export const ImageStore = makeFluxStore(getInitialState, reduce);
